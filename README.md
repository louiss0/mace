...
- Schema-validated runtime input through `parse = <Schema>` and `parse_file = "<path>"`, including remote schema files over HTTP(S); `parse_file` can infer the schema when the referenced file exports exactly one schema
