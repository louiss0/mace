...
- Schema-validated runtime input through `output = data, parse = <Schema>` and `output = data, parse_file = "<path>"`, including remote schema files over HTTP(S); both inject matching input fields into the output block as global variables, `parse` selects an already-available schema, and `parse_file` loads schema declarations and can infer the schema when the referenced file exports exactly one schema
