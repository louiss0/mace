...
[## Conformance](#mace-language-specification)
...
[## Terminology](#mace-language-specification)
...
[## Source Text](#mace-language-specification)
...
[## Lexical Grammar](#mace-language-specification)
...
schemas, enums, enum members, variables, imported symbols, and record fields.
...
`from`, `import`, `type`, `schema`, `enum`, `string`, `int`, `float`,
`boolean`, `array`, `union`, `variant`, `injectable`, `gen_doc`,
`schema_doc`, `true`, and `false`.

Directive words such as `output`, `data`, and `schema_file` are interpreted
inside directive lists.
...
- `[` and `]` for directive lists, array literals, array access, and type lists
...
- `=` for variable initializers, directive assignment, and explicit enum values
- `;` for declaration termination
- `,` for field and member separation
- `.` for member and enum access
- `?` for optional fields and conditional expressions
- `/#` for inline declaration descriptions

[## Syntactic Grammar](#mace-language-specification)
...
[## Static Semantics](#mace-language-specification)
...
schemas, enums, variables, and imports share one declaration namespace, so a  
name cannot be redeclared in another declaration category.
...
the value environment. Type aliases, schemas, and enum declarations are not  
runtime values.

Enum declarations expose member access expressions such as `Fruit.Apple`, but  
an enum declaration name alone is not a runtime value.
...
[## Type System](#mace-language-specification)
...
schema describes a closed record shape, a union composes schemas or enum  
domains, and a variant describes a closed set of alternatives.
...
Enums are nominal types. Two enums with the same backing type are not  
interchangeable unless explicitly composed through union semantics.
...
Union identity is determined by the resulting composed schema or enum domain.
...
- or assign raw backing values to enum-typed positions.
...
Enum assignability requires enum member values.

Variant assignability requires exactly one matching member type.

Union assignability depends on the composed schema or enum result.
...
- enum unions with incompatible backing types,
...
- accessing unknown enum members,
...
[## Evaluation Semantics](#mace-language-specification)
...
The document context has one identifier namespace. A type, schema, enum,
variable, or imported name cannot reuse a name that already exists in that
context. This means a variable cannot have the same name as a type, a schema
cannot have the same name as an enum, and a local declaration cannot shadow an
imported name.
...
declarations so imported names can be used by local types, schemas, enums, and
variables.
...
enums depending on the field type.
...
Enums evaluate as named scalar domains. An enum declaration does not emit a
record field by itself. It defines member values that can be selected through
member access, such as `Status.Ready`.

Variables evaluate by type, then value. The declared type is resolved first,
the initializer expression is evaluated, the resulting value may be coerced in
type-directed enum-union contexts, and then the value is validated against the
declared type before being added to the value environment.
...
appropriate category. `gen_doc` applies to type declarations and non-record
variables. `schema_doc` applies to schemas, enums, and record-valued variables.
If the target has not appeared before the documentation block, or the target is
not valid for the documentation kind, evaluation rejects the document.
...
alias, schema, or enum declaration as a plain value expression.
...
whose type resolves to an enum is importable as an enum. Other valid field
types are importable as type aliases.

[## Expression Evaluation](#mace-language-specification)
...
Member access evaluates either enum members or record fields. If the target is
an enum name, the member is resolved from the enum. Otherwise, the target
...
[## Operator Precedence](#mace-language-specification)
...
| ---------- | ------------------------------ | ------------- | -------------------------------- |
| 15         | `.`, `[]`                      | left          | member access, array access      |
...
| 3          | `&&`                           | left          | logical and                      |
| 2          | `\|\|`                         | left          | logical or                       |
| 1          | `? :`                          | right         | conditional expression           |
...
The conditional operator is right-associative. Nested conditionals group toward
the `else` branch unless parentheses choose another grouping.
...
[## Comparison and Equality Evaluation](#mace-language-specification)
...
[## Logical and Conditional Evaluation](#mace-language-specification)
...
[## Structural Merge Evaluation](#mace-language-specification)
...
expression resolves to a type alias, schema, or enum declaration instead of a
...
#[## Enum Validation](#mace-language-specification)

An enum must use one of the supported backing types: `string`, `int`, or
`float`. Enum member names must be unique. Enum member values must be unique
after implicit or explicit values are resolved. An enum must use either all
implicit members or all explicit members; mixing implicit and explicit members
is invalid.

Explicit enum values must be literals compatible with the enum backing type.
Values assigned to enum-typed declarations or schema fields must use enum
member access. Raw backing values are not assignable to enum types.
...
Union members must be all schemas or all enums. Conflicting schema fields are
invalid. Required fields remain required unless every union member marks the
field optional.

Enum unions require all member enums to use the same backing type. Enum union
members are merged according to Mace's enum union rules.

#[## Variant Validation](#mace-language-specification)

A variant value must match exactly one member type. If a value matches zero
members or more than one member, validation fails.
...
A `schema_doc` declaration must target an existing schema or enum. A target **MAY**
have at most one documentation declaration. Unknown documentation entries are
invalid. Duplicate documentation entries are invalid.
...
[## Ordered Model](#mace-language-specification)
...
Only names surfaced through the imported file's output block are importable. Top-level `type`, `enum`, `schema`, and variable declarations are internal unless surfaced through the output block.
...
- invalid array access targets
    
- out-of-range array indexes when statically known
...
Primitive types, array types, union types, variant types, enum types, schema types, and declared type aliases are resolved before expression type checking.
...
Enum values MUST be resolved through their declaring enum name.
...
> [!info] `mace.syntax.missing-output-block`
> 
...
> [!info] `mace.syntax.unexpected-token-after-script-block`
> 
...
> [!info] `mace.syntax.unexpected-token-after-output-block`
> 
...
> [!info] `mace.syntax.missing-output-opening-brace`
> 
...
> [!info] `mace.syntax.missing-output-closing-brace`
> 
...
> [!info] `mace.syntax.missing-script-opening-delimiter`
> 
...
> [!info] `mace.syntax.unterminated-script-block`
> 
...
> [!info] `mace.syntax.inconsistent-script-delimiters`
> 
...
> [!info] `mace.syntax.import-after-declaration`
> 
...
> [!info] `mace.syntax.malformed-import`
> 
...
> [!info] `mace.syntax.import-missing-from`
> 
...
> [!info] `mace.syntax.import-missing-path`
> 
...
> [!info] `mace.syntax.import-missing-import-keyword`
> 
...
> [!info] `mace.syntax.import-missing-name`
> 
...
> [!info] `mace.syntax.import-missing-name-after-comma`
> 
...
> [!info] `mace.syntax.import-missing-semicolon`
> 
...
> [!info] `mace.syntax.malformed-directive-list`
> 
...
> [!info] `mace.syntax.directive-missing-opening-bracket`
> 
...
> [!info] `mace.syntax.directive-missing-closing-bracket`
> 
...
> [!info] `mace.syntax.directive-missing-pair`
> 
...
> [!info] `mace.syntax.directive-output-missing-equals`
> 
...
> [!info] `mace.syntax.directive-output-invalid-value`
> 
...
> [!info] `mace.syntax.directive-schema-file-missing-equals`
> 
...
> [!info] `mace.syntax.directive-schema-file-missing-string`
> 
...
> [!info] `mace.syntax.directive-schema-missing-equals`
> 
...
> [!info] `mace.syntax.directive-schema-missing-name`
> 
...
> [!info] `mace.syntax.malformed-variable-declaration`
> 
...
> [!info] `mace.syntax.variable-missing-type`
> 
...
> [!info] `mace.syntax.variable-missing-name`
> 
...
> [!info] `mace.syntax.variable-inline-description-not-allowed`
> 
...
> [!info] `mace.syntax.variable-missing-semicolon`
> 
...
> [!info] `mace.syntax.malformed-type-declaration`
> 
...
> [!info] `mace.syntax.type-missing-name`
> 
...
> [!info] `mace.syntax.type-missing-colon`
> 
...
> [!info] `mace.syntax.type-missing-reference`
> 
...
> [!info] `mace.syntax.type-missing-semicolon`
> 
> Reported when a type declaration is missing `;`.
> 
> ```txt
...
> [!info] `mace.syntax.malformed-schema`
> 
...
> [!info] `mace.syntax.schema-missing-name`
> 
...
> [!info] `mace.syntax.schema-missing-colon`
> 
...
> [!info] `mace.syntax.malformed-enum`
> 
> Generic fallback for malformed enum declarations.
> 
> ```txt
> malformed enum declaration
> ```

> [!info] `mace.syntax.enum-missing-name`
> 
> Reported when an enum declaration is missing its identifier.
> 
> ```txt
> expected enum name
> ```

> [!info] `mace.syntax.enum-missing-colon`
> 
> Reported when an enum declaration is missing `:`.
> 
> ```txt
> expected ':' after enum name
> ```

> [!info] `mace.syntax.enum-missing-backing-type`
> 
> Reported when an enum declaration is missing a backing type.
> 
> ```txt
> expected enum backing type
> ```

> [!info] `mace.syntax.enum-missing-opening-brace`
> 
> Reported when an enum declaration does not begin with `{`.
> 
> ```txt
> expected '{' to start enum declaration
> ```

> [!info] `mace.syntax.enum-missing-closing-brace`
> 
> Reported when an enum declaration is missing `}`.
> 
> ```txt
...
> [!info] `mace.syntax.enum-missing-member-name`
> 
> Reported when an enum member identifier is missing.
> 
> ```txt
> expected enum member name
> ```

> [!info] `mace.syntax.enum-invalid-member-value`
> 
> Reported when an enum member value is malformed.
> 
> ```txt
> expected enum member value
> ```

### Type Diagnostics

> [!info] `mace.syntax.malformed-type-reference`
> 
...
> [!info] `mace.syntax.record-type-missing-opening-brace`
> 
...
> [!info] `mace.syntax.record-type-missing-closing-brace`
> 
...
> [!info] `mace.syntax.record-field-missing-name`
> 
...
> [!info] `mace.syntax.record-field-missing-colon`
> 
...
> [!info] `mace.syntax.record-field-missing-type`
> 
...
> [!info] `mace.syntax.array-type-missing-opening-angle`
> 
...
> [!info] `mace.syntax.array-type-missing-closing-angle`
> 
...
> [!info] `mace.syntax.variant-type-missing-closing-bracket`
> 
...
> [!info] `mace.syntax.missing-expression`
> 
...
> [!info] `mace.syntax.unclosed-grouping`
> 
...
> [!info] `mace.syntax.self-missing-dot`
> 
...
> [!info] `mace.syntax.self-missing-field`
> 
...
> [!info] `mace.syntax.array-literal-missing-closing-bracket`
> 
...
> [!info] `mace.syntax.record-literal-missing-closing-brace`
> 
...
> [!info] `mace.syntax.record-field-missing-separator`
> 
...
> [!info] `mace.syntax.member-access-missing-name`
> 
...
> [!info] `mace.syntax.invalid-array-access-index`
> 
> Reported when array access does not use an integer index.
> 
> ```txt
> expected integer index in array access
> ```

> [!info] `mace.syntax.array-access-missing-closing-bracket`
> 
> Reported when array access is missing `]`.
> 
> ```txt
> expected ']' after array access index
> ```

> [!info] `mace.syntax.conditional-missing-colon`
> 
...
> [!info] `mace.syntax.malformed-output-field`
> 
...
> [!info] `mace.syntax.output-field-missing-name`
> 
...
> [!info] `mace.syntax.output-field-missing-colon`
> 
...
> [!info] `mace.syntax.output-field-missing-value`
> 
...
> [!info] `mace.syntax.output-field-missing-separator`
> 
...
> [!info] `mace.doc.malformed-gen-doc`
> 
...
> [!info] `mace.doc.malformed-schema-doc`
> 
...
> [!info] `mace.doc.inline-description-not-allowed`
> 
...
> [!info] `mace.doc.duplicate-inline-description`
> 
...
> [!info] `mace.doc.output-doc-requires-directive`
> 
...
> [!info] `mace.doc.output-doc-must-be-block-string`
> 
...
> [!info] `mace.syntax.unexpected-token`
> 
...
Mace uses named-only imports. An imported file does not expose every declaration in its script block. Only symbols surfaced through the imported file’s output block are importable. Top-level `type`, `enum`, `schema`, and variable declarations remain internal unless surfaced through output.
...
Mace expressions are pure and deterministic. The implementation supports identifiers, member access, array access, literals, array literals, record literals, `$self`, unary operators, arithmetic operators, comparison operators, equality operators, bitwise operators, logical operators, and ternary conditionals.
