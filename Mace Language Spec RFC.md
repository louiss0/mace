...
Inside a schema or inline record type, type-position `$self` denotes the
currently enclosing record shape. A recursive reference MUST be structurally
guarded by `array<...>` or `record<...>`; direct recursion and use outside a
record type are errors. Each nested inline record establishes a new `$self`
scope. For example:
...
The schema and inline variable above both describe records whose children have
the same shape as their enclosing record.
