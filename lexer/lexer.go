...
func (l *Lexer) shouldUseBlockComment() bool {
	...
	closingIndex := indexOf(remaining, "=/")
	if closingIndex == -1 {
		return false
	}

	newlineIndex := indexOfNewline(remaining)
	if newlineIndex == -1 {
		return true
	}

	return closingIndex < newlineIndex
}
...
func (l *Lexer) match(expected byte) bool {
	if l.isAtEnd() || l.input[l.position] != expected {
		return false
	}
	l.advance()
	return true
...
func indexOfNewline(value string) int {
