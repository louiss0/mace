...
func TestCodecCoveragePackageBranches(t *testing.T) {
	...
	_, err = jsonSchemaChoiceType([]any{true})
	require.Error(t, err)
	_, _, err = jsonSchemaChoiceValues([]any{"x", int64(1)})
	require.Error(t, err)
	_, _, err = jsonSchemaChoiceValues([]any{1.2})
	require.Error(t, err)
...
func TestCodecCoverageRemainingBranches(t *testing.T) {
	...
	_, _, err = jsonSchemaChoiceValues([]any{})
	require.Error(t, err)
