...
var directiveKeywordDocs = map[string]string{
	...
	"parse":       "Validates the runtime `input` record against a schema already available in the current context and injects matching fields into the output block as global variables.",
	"parse_file":  "Loads schema declarations from another Mace file for parse-driven validation. When used alone, the referenced file must expose exactly one schema. Matching runtime input fields are injected into the output block as global variables, just like `parse = <Schema>`. It still parses the runtime `input` record, not external JSON, YAML, or TOML data.",
}
